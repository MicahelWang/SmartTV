...
namespace YeahTVApi.Client
{
    ...
    public class HttpClient
    {
        ...
        private HttpWebResponse GetResponseByJsonContentType(string url, string method, string data, string host, string orign, string refer)
        {
            HttpWebResponse response = null;
            Stream st = null;
            HttpWebRequest req = null;
        ...
        public string PostJson(string url, string data = "", string host = "", string orign = "", string refer = "", string method = "post")
        {
            return GetStringByJsonContentType(method, url, data, host, orign, refer);
        }
        public string PostGetApiObjectResult(string url, byte[] key, string data = "", string host = "", string orign = "", string refer = "", string method = "post")
        {
            return GetApiObjectResult(method, url, data, host, orign, refer, key);
        }
        #endregion
        ...
        private string GetStringByJsonContentType(string method, string url, string data, string host, string orign, string refer)
        {
            var result = string.Empty;
            var response = GetResponseByJsonContentType(url, method, data, host, orign, refer);
            if (response != null)
            {
                var stream = response.GetResponseStream();
                if (response.ContentEncoding.ToLower().Contains("gzip"))
        ...
        private string GetApiObjectResult(string method, string url, string data, string host, string orign, string refer, byte[] key)
        {
            string result=null ;
            var response = GetResponse(url, method, data, host, orign, refer);
            if (response != null)
            {
                ...
                result = PubFun.JsonStreamToJsonString(stream, true, key);
            }
            return result;
        }
        ...
        public static string EncodingParams(string str)
        {
            ...
            return System.Web.HttpUtility.UrlEncode(str, Encoding.GetEncoding("gb2312")).ToUpper();
        }

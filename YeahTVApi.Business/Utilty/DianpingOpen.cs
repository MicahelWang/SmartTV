...
public class DianpingOpen
{
    ...
    public static string MakeRequest(string MethodName, SortedList<string, string> list)
    {
        ...
        //组装url
        string url =HZTVApi.Common.PubFun.GetAppSetting("dianping.apiurl") + MethodName;
        //组装参数
        if (list == null)
        {
        ...
        String apiKey = HZTVApi.Common.PubFun.GetAppSetting("dianping.apikey");
        StringBuilder sb = new StringBuilder();
        sb.Append(apiKey);
        sb.Append(value);
        sb.Append(HZTVApi.Common.PubFun.GetAppSetting("dianping.apiSecret"));
        value = sb.ToString();
        try
        {
    ...
    public static IList<DShopEntity> GetShops( string regionid, string categoryid, string keyword, string price, string hascoupon, string latitude, string longitude, string radius, string sort, string desc, string limit)
    {
        ...
        list["category"] = HZTVApi.Common.PubFun.GetAppSetting("dianping.category." + categoryid);
        list["limit"] = limit;
        list["format"] = "json";
        list["offset_type"] = "1";
